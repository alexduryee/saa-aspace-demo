      ...
      xml.ead(                  'xmlns:xsi' => 'http://www.w3.org/2001/XMLSchema-instance',
        ...
        xml.archdesc(atts) {
          ...
          xml.did {
            ...
            # Plugin modification - add type="local_mss" to the collection-level <unitid> element
            # This maps to the Identitifer field in the ASpace staff interface
            xml.unitid((0..3).map{|i| data.send("id_#{i}")}.compact.join('.'), :type => 'local_mss')
    ...
    xml.send(tag_name, atts) {
      ...
      xml.did {
        ...
        if !data.component_id.nil? && !data.component_id.empty?
          # Plugin modification - add type="local_mss" to component-level <unitid> elements
          # This maps to the Component Unique Identifier field in the ASpace staff interface
          xml.unitid(data.component_id, :type => 'local_mss')
        end
